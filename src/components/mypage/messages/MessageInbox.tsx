...
export default function MessageInbox() {
  ...
  const [receivedMessages, setReceivedMessages] = useState<MessageWithRelations[]>([]);
    ...
    <div className="space-y-4">
      ...
      {receivedMessages.length === 0 ? (
        ...
        receivedMessages.map((message) => (
          <div key={message.id} className="bg-gray-100 p-4 rounded-lg shadow-sm">
            <p className="text-sm text-gray-600">
              <span className="font-semibold">{message.sender.name || '익명'}</span> 님으로부터
            </p>
