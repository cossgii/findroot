...
interface KakaoMapProps extends HTMLAttributes<HTMLDivElement> {
  ...
  polylines?: { path: { lat: number; lng: number }[]; strokeColor?: string; strokeOpacity?: number; strokeWeight?: number; }[]; // New prop for drawing polylines
}

const KakaoMap = forwardRef<HTMLDivElement, KakaoMapProps>(
  ({ latitude, longitude, markers = [], selectedMarkerIds, onMarkerClick, polylines, ...rest }, ref) => {
      ...
      if (window.kakao && window.kakao.maps && mapContainerRef.current) {
        window.kakao.maps.load(() => {
          ...
          markers.forEach((markerData) => {
            ...
            let markerImageSrc = 'https://t1.daumcdn.net/mapapidoc/marker_number_blue.png'; // Default marker
            let markerSize = new window.kakao.maps.Size(36, 37);
            let spriteOrigin = new window.kakao.maps.Point(0, 0);
            let spriteSize = new window.kakao.maps.Size(36, 37);

            // Example: Change marker for selected items (you can customize this further)
            if (selectedMarkerIds?.includes(markerData.id)) {
              markerImageSrc = 'https://t1.daumcdn.net/mapapidoc/marker_number_red.png'; // Selected marker
            }
            ...
            if (markerData.category && categoryMarkerImages[markerData.category]) {
                markerImageSrc = categoryMarkerImages[markerData.category];
            }

            // Override if selected
            if (selectedMarkerIds?.includes(markerData.id)) {
              markerImageSrc = 'https://t1.daumcdn.net/mapapidoc/marker_yellow.png'; // Selected marker (e.g., yellow)
            }
          ...
          polylines?.forEach(lineData => {
            const path = lineData.path.map(p => new window.kakao.maps.LatLng(p.lat, p.lng));
    ...
    }, [latitude, longitude, markers, selectedMarkerIds, onMarkerClick, polylines]); // Add polylines to dependencies
